      ...
      >
        ...
        >
          <VueDragResize
            ...
            v-on:dragging="changeDragSize($event, index)"
            v-on:resizing="changeDragSize($event, index)"
          >
            <component
              :key="item.id"
              :is="item.type"
            ...
            <div class="component-info" v-if="active && componentData">
              <el-collapse>
                ...
                <el-collapse-item title="标题设置">
                  ...
                  ></el-color-picker> -->
                      <input type="color" />
                      <!-- <el-color-picker v-model="componentData.title.textStyle.color" /> -->
                    </el-form-item>
            ...
            <div class="data-info" v-if="active && componentData">
              <el-form label-position="left" label-width="100px" size="mini">
                ...
                <el-form-item label="刷新间隔">
                  <el-input-number
                    :min="0"
                    v-model="componentData.dataRefresh"
                  ></el-input-number>
                </el-form-item>
              </el-form>
            </div>
...
import { echartsData } from "@/components/c/getData";
export default {
  name: "app",
  components: {},
  data() {
  ...
  mounted() {},
  computed: {
    ...mapGetters(["pageData", "componentList", "active", "activeIndex"]),
    echartList() {
      let arr = [];
      ...
      handler(val) {
        if (val) {
          document.onkeydown = (e) => {
            ...
            if (e.code == "Delete") {
              this.$store.dispatch("edit/removeComponent", {
  ...
  methods: {
    ...
    dragend(params) {
      ...
      this.$store.dispatch("edit/addComponent", {
        component: {
          ...
          active: true,
          type: params.type,
          options: deepClone(data.options),
          dataSource: "",
          dataRefresh: 0,
        },
      });
    ...
    activateEv(index) {
      ...
      this.$store.dispatch("edit/changeZToTop", { index: index });
    },
